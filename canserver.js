const expressServer = require('express'),
  ...
  canDumper = require('./can-dumper.js').dumper,
  logger = require('./logger.js').logger,
  serverLogger = require('./serverLogger.js').serverLogger,
...
try {
  ...
  channel.addListener("onMessage", (msg) => {
    ...
    // save to logger if logging
    if (logger.logging()) {
      logger.log(logger.TYPE.CAN, msg);
    }
  });
...
try {
  ...
  // // SHOW CODE; reboot every four hours
  // setTimeout(() =>{
  ...
  //     }
  // });
  // }, 1000 * 120);  // 1000 * 60 * 60 * 4 after 4 hours
} catch (error) {
  serverLogger.add(error.toString());
}

serverLogger.add('!! ----------- CAN-SERVER Ready ----------- !!');
